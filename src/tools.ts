...
export const signJwt = (
  ...
  const defaultHeader: Partial<JwtHeader> = { kid, alg: "RS256" };
  if (jku != null) {
    defaultHeader.jku = jku;
  }
  const header =
    jwtPayload.header != null
      ? { ...defaultHeader, ...jwtPayload.header }
      : defaultHeader;
  return JWT.sign(jwtPayload, forge.pki.privateKeyToPem(privateKey), {
    header,
  });
};
